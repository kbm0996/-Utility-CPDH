...
#define df_PDH_CPUUSAGE_TOTAL L"\\Processor(_Total)\\% Processor Time"	// CPU 전체 사용률(%)
#define df_PDH_CPUUSAGE_0 L"\\Processor(0)\\% Processor Time"			// CPU 코어 사용률(%)
...
//#define df_PDH_CPUUSAGE_USER L"\\Process(NAME)\\% User Time"			// 프로세스 CPU 유저 사용률(%)
//#define df_PDH_CPUUSAGE_USER L"\\Process(NAME)\\% Processor Time"		// 프로세스 CPU 전체 사용률(%)
/// MEMORY ///
#define df_PDH_MEMINUSE_BYTES L"\\Memory\\Committed Bytes" // 미리 할당된 메모리(Bytes)
#define df_PDH_MEMAVAIL_BYTES L"\\Memory\\Available Bytes" // 사용가능 메모리(Bytes)
#define df_PDH_MEMAVAIL_KB L"\\Memory\\Available KBytes" // 사용가능 메모리(Kilobytes)
#define df_PDH_MEMAVAIL_MB L"\\Memory\\Available MBytes" // 사용가능 메모리(Megabytes)
#define df_PDH_MEMINUSE_PERCENT L"\\Memory\\% Committed Bytes In Use" // 사용중 메모리(Committed Bytes/Commit Limit)(%)
#define df_PDH_MEMLIMIT_BYTES L"\\Memory\\Commit Limit" // 미리 할당 한계 메모리(Bytes)
#define df_PDH_NONPAGEDMEM_BYTES L"\\Memory\\Pool Nonpaged Bytes" // 논페이지 메모리(Bytes)
/// PROCESS ///
#define df_PDH_PROCESS_COUNT(PROCESS_NAME) L"\\Process("#PROCESS_NAME")\\Thread Count" // 프로세스 스레드 수(n)
#define df_PDH_PROCESS_HANDLE_COUNT(PROCESS_NAME) L"\\Process("#PROCESS_NAME")\\Handle Count" // 프로세스 핸들 수(n)
#define df_PDH_PROCESS_VRMEM_BYTES(PROCESS_NAME) L"\\Process("#PROCESS_NAME")\\Virtual Bytes" // 프로세스 가상 메모리(Bytes)
// * 가상 메모리 = 예약된 메모리(Reserved) + 경계(boundary, 65KB). * 실제 사용 메모리(commit)과는 무관
#define df_PDH_PROCESS_USERMEM_BYTES(PROCESS_NAME) L"\\Process("#PROCESS_NAME")\\Private Bytes" // 프로세스 유저할당 메모리(Bytes)
// * 프로세스 공용 메모리/커널 메모리 제외
#define df_PDH_PROCESS_WORKMEM(PROCESS_NAME) L"\\Process("#PROCESS_NAME")\\Working Set" // 프로세스 작업 메모리(Bytes?)
// * 현재 물리 메모리에 사용되는 크기. 실제 할당 용량은 아닐 수 있음
#define df_PDH_PROCESS_NPMEM(PROCESS_NAME) L"\\Process("#PROCESS_NAME")\\Pool Nonpaged Bytes" // 프로세스 논페이지 메모리(Bytes)
/// ETHERNET ///
...
public:
	...
	//  쿼리 카운터 추가
	//
	// Parameters : (WCHAR*) 쿼리(CPdh.h 내 define값 참고)
